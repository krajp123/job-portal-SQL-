                    ...
                    {!activeId ? (
                        ...
                        <>
                            ...
                            {canReply ? <div className="flex items-center gap-2 border-t border-[#F0D1BF] bg-white p-2 sm:p-3">
                                <textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { const isTouchDevice = window.matchMedia('(pointer: coarse)').matches; if (event.key === 'Enter' && !event.shiftKey && !isTouchDevice) { event.preventDefault(); sendReply(); } }} placeholder={`Reply to ${activeName}`} rows={1} className="max-h-24 min-h-10 min-w-0 flex-1 resize-none rounded-2xl border border-[#EBC2AE] bg-[#FFFDFC] px-4 py-2.5 text-sm leading-5 outline-none focus:border-[#C75560]" />
